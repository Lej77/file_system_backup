//! Simple in-memory file system forked from
//! [`dav_server::memfs::MemFs``](https://docs.rs/dav-server/0.5.7/src/dav_server/memfs.rs.html)
//!
//! This implementation has state, so if you create a new instance in a
//! handler(), it will be empty every time.
//!
//! This means you have to create the instance once, using `MemFs::new`, store
//! it in your handler struct, and clone() it every time you pass it to the
//! DavHandler. As a MemFs struct is just a handle, cloning is cheap.
...
impl MemFs {
    /// Create a new "memfs" file system.
...
impl DavFileSystem for MemFs {
        ...
        async move {
            ...
            for (name, dnode_id) in tree.get_children(node_id)? {
                if let Ok(node) = tree.get_node(dnode_id) {
                    v.push(Ok(Box::new(node.as_dirent(&name))));
                }
            }
    ...
    fn create_dir<'a>(&'a self, path: &'a DavPath) -> FsFuture<'a, ()> {
        async move {
            log::trace!("FS: create_dir {:?}", path);
            let tree: &mut tree::Tree<Vec<u8>, MemFsNode> = &mut self.tree.lock().unwrap();
    ...
    fn get_quota(&self) -> FsFuture<'_, (u64, Option<u64>)> {
        async move {
            ...
            let total_data = tree
                .all_nodes()
                .map(|node| {
                    if let MemFsNode::File(file) = &node.data {
        ...
        async move {
            ...
            if offset < 0 {
                if -offset as u64 > start {
                    return Err(fs_error_from_io_error(&Error::new(
...
fn fs_error_from_io_error(e: &Error) -> FsError {
