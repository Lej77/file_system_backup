...
- The `backup` command works without any 3rd party program and will manually scan the file system to gather the required information.
  - Warning: this is not very well tested and might have bugs.
  - This command supports fast [MFT](https://en.wikipedia.org/wiki/NTFS#Master_File_Table) scanning for NTFS disks if permissions allow
    - This is a [standout feature of `WizTree`](https://www.diskanalyzer.com/wiztree-vs-windirstat), so there is more information there.
    ...
    - On Linux you can add your user to the `disk` group to access NTFS block devices. Though this would also allow other programs read and write access to block devices.
    - On Linux `sudo setcap cap_dac_read_search+ep /path/to/program` should allow access to block devices for only a specific program.
...
- The `mount` feature can read a backup file and create a "fake" filesystem that can be browsed to navigate the stored information.
